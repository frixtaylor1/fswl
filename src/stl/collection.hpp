...
struct Collection {
    ...
    Iterator       begin(uint32 idx = 0);
    const Iterator begin(void) const;
    Iterator       end(void);
    Iterator       end(uint32 idx);
    const Iterator end(void) const;
    
    Collection();
    Collection(const Collection< ItemType, CAPACITY >& collection); 
    Collection(ItemType items[], uint32 nbItems);
    
    void              init(const ItemType items[], uint32 nbItems);
    void              reset(void);
    void              release(void);
    ...
    bool              isFull(void);
    bool              isEmpty(void);
    ItemType&         at(uint32 idx);
    const ItemType&   at(uint32 idx) const;
    ...
    template< uint32 NewCapacity >
    Collection< ItemType, NewCapacity > slice(uint32 pos, uint32 nbItems);
    
    /**
     * Modify family functions...
     */
    const ItemType&   add(const ItemType& item);
    const Collection& add(const ItemType items[], uint32 nbItems);
    Collection&       add(Collection& rhs);
    const ItemType&   addAt(uint32 idx, const ItemType& item);
    bool              tryAdd(const ItemType& item);
    bool              tryAdd(const ItemType items[], uint32 nbItems);
    ...
    inline void _ensureIndexLengthInBounds(uint32 idx) const;
    inline void _ensureIndexLengthInBounds(const uint32& idx);
    inline void _ensureIndexCapacityInBounds(uint32 idx) const;
    inline void _ensureCapacity(uint32 nbItems);
    inline void _ensureCapacityAddingArray(uint32 nbItems);
    inline void _ensureCapacityAddingArrayWithoutFail(uint32 nbItems);
...
template< class ItemType, uint32 CAPACITY >
uint32 Collection< ItemType, CAPACITY >::Iterator::currentPos() const { 
    return pos; 
}
...
template< class ItemType, uint32 CAPACITY >
Collection< ItemType, CAPACITY >::Iterator Collection< ItemType, CAPACITY >::begin(uint32 idx) { 
...
template< class ItemType, uint32 CAPACITY >
Collection< ItemType, CAPACITY >::Iterator Collection< ItemType, CAPACITY >::end(uint32 idx) { 
    return Collection< ItemType, CAPACITY >::Iterator(&items[idx]); 
}
...
template< class ItemType, uint32 CAPACITY >
Collection< ItemType, CAPACITY >::Collection(ItemType items[], uint32 nbItems): length(0), currentItemPos(0), initialized(true) {
    init(items, nbItems);
}

template< class ItemType, uint32 CAPACITY >
void Collection< ItemType, CAPACITY >::init(const ItemType items[], uint32 nbItems) {
    _ensureCapacity(nbItems);
    initialized = true;

    for (uint32 idx = 0; idx < nbItems; idx++) {
        add(items[idx]);
    }
...
template< class ItemType, uint32 CAPACITY >
ItemType& Collection< ItemType, CAPACITY >::at(uint32 idx) {
    _ensureIndexLengthInBounds(idx);
    return items[idx];
}

template< class ItemType, uint32 CAPACITY >
const ItemType& Collection< ItemType, CAPACITY >::at(uint32 idx) const {
    _ensureIndexLengthInBounds(idx);
    return items[idx];
}
...
template< class ItemType, uint32 CAPACITY >
const Collection< ItemType, CAPACITY >& Collection< ItemType, CAPACITY >::add(const ItemType items[], uint32 nbItems) {
...
template< class ItemType, uint32 CAPACITY >
const ItemType& Collection< ItemType, CAPACITY >::addAt(uint32 idx, const ItemType& item) {
...
template< class ItemType, uint32 CAPACITY >
bool Collection< ItemType, CAPACITY >::tryAdd(const ItemType items[], uint32 nbItems) {
...
template< class ItemType, uint32 CAPACITY >
template< uint32 NewCapacity >
Collection< ItemType, NewCapacity > Collection< ItemType, CAPACITY >::slice(uint32 pos, uint32 nbItems) {
...
template< class ItemType, uint32 CAPACITY >
void Collection< ItemType, CAPACITY >::removeAt(uint32 idx) {
    _ensureIndexLengthInBounds(idx);
...
template< class ItemType, uint32 CAPACITY >
inline void Collection< ItemType, CAPACITY >::_ensureIndexLengthInBounds(uint32 idx) const {
    SA_ASSERT(idx < length, "Index out of bounds!");
}

template< class ItemType, uint32 CAPACITY >
inline void Collection< ItemType, CAPACITY >::_ensureIndexLengthInBounds(const uint32& idx) {
    SA_ASSERT(idx < length, "Index out of bounds!");
}

template< class ItemType, uint32 CAPACITY >
inline void Collection< ItemType, CAPACITY >::_ensureIndexCapacityInBounds(uint32 idx) const {
    SA_ASSERT(idx < MAX_COLLECTION_CAPACITY, "Index out of bounds!");
}

template< class ItemType, uint32 CAPACITY >
inline void Collection< ItemType, CAPACITY >::_ensureCapacity(uint32 nbItems) {
    SA_ASSERT(nbItems <= MAX_COLLECTION_CAPACITY, "Error not enough capacity!");
}

template< class ItemType, uint32 CAPACITY >
inline void Collection< ItemType, CAPACITY >::_ensureCapacityAddingArray(uint32 nbItems) {
    _ensureIsInitialized();
    SA_ASSERT(nbItems + length <= DEFAULT_CAPACITY, "Index out of bound!");
}

template< class ItemType, uint32 CAPACITY >
inline void Collection< ItemType, CAPACITY >::_ensureCapacityAddingArrayWithoutFail(uint32 nbItems) {
